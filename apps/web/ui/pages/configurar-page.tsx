...
export function ConfigurarPage() {
  ...
  const [regiao, setRegiao] = useState('Brasília, DF');
  const [palavras, setPalavras] = useState('equipamentos, informática, software, TI');
  ...
  const podeEditarCriterios = pode('CRITERIO_MONITORAMENTO', 'escrever');
  const podeEditarPreferencias = pode('PREFERENCIA_NOTIFICACAO', 'escrever');
        ...
        <div style={{ ...cardStyle, flex: '1.5' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--radar-space-5)' }}>
            <Input
              ...
              hint="Código CNAE da atividade principal (ex.: 6201-5/01 — Desenvolvimento de programas)"
            ...
            <div>
              <label style={labelStyle}>Valor estimado (faixa)</label>
              <div style={{ display: 'flex', gap: 'var(--radar-space-3)', alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  <Input placeholder="R$ 10.000" />
                </div>
                <span style={{ color: 'var(--radar-color-text-muted)', fontSize: 'var(--radar-font-size-sm)', flexShrink: 0 }}>até</span>
                <div style={{ flex: 1 }}>
                  <Input placeholder="R$ 500.000" />
                </div>
              </div>
            </div>
