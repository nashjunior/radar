...
> Define **como o front é feito**. O `apps/web` roda com **React 19 + Vite 6** (SPA estática, sem Next.js), estruturado em Clean Architecture (`domain/application/infra/ui`) e consumindo os design tokens agnósticos de `shared/design-tokens` (P-75). Decisão confirmada pela implementação em produção — P-77 **Resolvido**. Consistente com a Clean Architecture do backend ([A10](10-padroes-e-estrutura-de-codigo.md)) e com o monorepo `modules/ + shared/`.
...
└─ apps/                  # front(s) — NOVO
   └─ web/                # SPA · React 19 + Vite 6 · sem Next.js · Clean Arch:
...
`design-tokens` é **agnóstico**: não sabe o que é React ou Angular. O `apps/web` importa o build CSS via `@import '@radar/design-tokens/tokens.css'` em `src/globals.css` e usa as CSS custom properties (`var(--radar-color-*)`) nos componentes. A **UI importa os use cases** de `application` via `UseCasesContext` (React Context — `ui/providers/use-cases-provider.tsx`) e **nunca** chama `infra`/API direto; o container de DI (`infra/container.ts`) compõe use cases com os gateways HTTP e expõe o conjunto via `UseCasesContext.Provider` em `src/main.tsx`. O `infra/` fala com o backend pelos contratos REST do BFF (`apps/api`). Todo use case é **abortável** (recebe `AbortSignal`, [A10 §1](10-padroes-e-estrutura-de-codigo.md)) — o hook React cria um `AbortController` no `useEffect` e chama `controller.abort()` no cleanup (§3.2). Estrutura `apps/` **confirmada** — P-77 Resolvido (2026-07-10).
...
```ts
// ui · React — useEffect cleanup cancela ao desmontar ou quando deps mudam (P-77 Resolvido)
useEffect(() => {
  const controller = new AbortController();
  obterTriagemUseCase.executar(input, controller.signal)
    .then((viewModel) => { if (!controller.signal.aborted) setState(viewModel); })
    .catch((err) => { if (controller.signal.aborted) return; /* tratar erro */ });
...
- ~~**Framework do front** e como consome os tokens; confirmar `apps/` no monorepo.~~ **Resolvido** (2026-07-10, P-77): **React 19 + Vite 6**; `apps/web` com Clean Arch `domain/application/infra/ui`; tokens via `@radar/design-tokens/tokens.css`; use cases via `UseCasesContext`; `AbortController` no `useEffect` cleanup.
