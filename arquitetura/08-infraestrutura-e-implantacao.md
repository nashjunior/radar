...
**Teto por tenant implementado na aplicação (P-55, RAD-209).** `rateLimitPorTenantMiddleware` (`apps/api/src/security.ts`) roda depois do `autenticarMiddleware` em cada rota — chave é o `tenantId` já verificado, nunca header do cliente. Contador em memória de processo (sem Redis na IaC hoje): o teto global por tenant é dividido pelo maior `max_capacity` entre os stacks e aplicado por task, então em scale-out completo o agregado converge para o teto pretendido; com menos tasks ativas o teto efetivo fica mais apertado — folga aceita no MVP-Now (detalhe em docs/98 P-55).
