...
- **`shared/kernel/` — o mínimo compartilhado** (documento 13: `tenantId`/`clienteFinalId` como *Shared Kernel*), IDs e classes-base de VO/erro, além de **contratos técnicos de infra genéricos** (tipos estruturais puros, zero vocabulário de domínio, usados só como parâmetro de construtor de adapters — ex. `DbClient`). Por linguagem (ex.: `ts/`). Manter **mínimo** — é o único acoplamento permitido entre contextos.
  ...
  - `db.ts` (RAD-193): `DbClient` — `{ query<R>(sql, params, opts?: { signal? }): Promise<{ rows: R[] }> }`, contrato mínimo de query SQL. Confirmado byte-a-byte idêntico em 12 arquivos `postgres-*`/`composicao.ts` de 4 módulos (ingestão/matching/notificação/triagem) antes da extração — nome tech-agnóstico mantido (não `PostgresClient`), interface não exportada localmente em nenhum adapter.
