...
> Tenant, usuário, cliente-final, o perfil de habilitação da empresa e o papel do usuário. Agregados: **Tenant**, **PerfilDeHabilitação**, **AtribuiçãoDePapel** (raiz própria, identificada pelo `sub` do IdP; documento 13, §3). `tenantId` é o *Shared Kernel* (documento 13, §5).
...
| **`ProvisionarOrganizacaoUseCase`** (P-109 L3 / RAD-283) | Usuário recém-cadastrado (onboarding pós-login, após o self-signup no IdP) | `{sub, email, cnpj, razaoSocial}` → `OrganizacaoDTO {tenantId, cnpj, razaoSocial, papel}` | `TenantRepository` (unicidade **1 CNPJ = 1 tenant**), `PermissaoRepository`, `EventPublisher` | `CnpjInvalidoError` (DV), `OrganizacaoJaExisteError` (CNPJ já vinculado), `UsuarioJaVinculadoError` (idempotente: devolve a organização existente) | `organizacao.provisionada` |
| **`ResolverContextoAutorizacaoUseCase`** | interno (borda, por requisição) / `GET /api/me` | `{usuarioId, tenantId}` → `ContextoAutorizacaoDTO {usuarioId, tenantId, papel, clienteFinalIds[]}` | `PermissaoRepository` | `AcessoNegadoError` (sem atribuição, ou `tenantId` da atribuição ≠ do token) | — |
| **`AutorizarAcessoUseCase`** | interno (toda requisição) | `{contexto: ContextoAutorizacaoDTO, recurso, acao, clienteFinalId?}` → `void` | `PermissaoRepository`, matriz declarativa `podeExecutar(papel, recurso, acao)` (documento 05, §4) | `AcessoNegadoError` | — |
...
**Invariantes de RBAC (P-52, decisão de arquitetura sobre a decisão de Produto+Segurança):** os dois controles são **cumulativos** — `AutorizarAcessoUseCase` responde *"este papel pode tentar esta ação"* (matriz do documento 05, §4) e a **autorização por objeto** (AB1/P-51), que continua **dentro de cada use case do contexto dono**, responde *"este usuário tem posse deste objeto"*. Nenhum substitui o outro e a ausência de papel **nega por padrão**. Três invariantes de implementação: **(1) papel — e, desde RAD-283, também o `tenantId` — não vêm do token** — o token OIDC carrega apenas **identidade verificada** (`sub`); papel, escopo de `clienteFinalId` **e o `tenantId`** são **dado de domínio** de Identidade & Organização (documento 13, §5), lidos por `PermissaoRepository` chaveado pelo `sub` da `AtribuiçãoDePapel`. **Derivar do `sub` não afrouxa o controle de documento 05, §2** — o `sub` é claim verificada e o tenant sai do *nosso* banco, nunca de header ou campo controlável pelo cliente. `custom:tenantId`, **quando presente** (contas nascidas de `AdminCreateUser`), é **cross-check**: divergir da atribuição ⇒ `AcessoNegadoError`. A claim deixou de ser pré-condição da sessão porque **ela é imutável e só populável na criação do usuário no Cognito** (P-109/RAD-283), e o Tenant de um usuário de self-signup só nasce **depois**, no `ProvisionarOrganizacaoUseCase`; sessão autenticada **sem** atribuição não é 403 cego, é o estado **"sem organização"** (`403 SEM_ORGANIZACAO`), que direciona ao onboarding — só as rotas `GET /api/me` e a de provisionamento são isentas de tenant. Isso mantém revogação/reatribuição de papel efetiva **sem novo login**, a relação N:M operador↔`clienteFinalId` fora do JWT e **nenhum atributo de negócio no IdP** (P-98). **(2) RBAC mora na borda** — a checagem recurso × ação é middleware de `apps/api`, **não** vaza para o domínio dos outros contextos (Matching, Triagem, Notificação não conhecem `Papel`); o que atravessa para dentro é o escopo `clienteFinalIds[]`, que já é linguagem do domínio. **(3) negação é auditável** — `AcessoNegadoError` por papel mapeia para `403` sem vazar existência do objeto e é registrada (documento 05, §3). A prova é a **matriz AB2** (arquitetura/07), teste obrigatório do gate. A **administração** de usuários/papéis (CRUD) não é pré-requisito da checagem: no *Now* a atribuição é semeada/provisionada, e a tela de gestão segue como escopo posterior (depende de design; documento 09).
