...
3. **Governança como Open Host.** Todo contexto publica proveniência/auditoria para a Governança, que centraliza base legal, direitos do titular e retenção (documentos 02, 05) — em vez de cada um reimplementar conformidade.
4. **Identidade como fornecedor upstream.** `tenantId` é o Shared Kernel mínimo; o Perfil de Habilitação é consumido pela Triagem via Cliente-Fornecedor.
...
- Validar os limites de contexto e a linguagem ubíqua com o time (glossário do documento 06 é o ponto de partida). `[A VALIDAR]`
- Confirmar se Governança & Conformidade é contexto separado ou *shared kernel* de conformidade. `[A VALIDAR]`
- Decidir onde vive o Perfil de Habilitação (Identidade vs. contexto próprio). `[A VALIDAR]`
