# Stack staging — infra/terraform (rewrite provider-agnóstico RAD-181/RAD-182, swapado 2026-07-11)
...
module "registry" {
  ...
  repository_name      = "api"
  encryption_key_ref   = var.kms_key_arn
  image_tag_mutability = "MUTABLE" # re-push da mesma tag no ciclo de validação
}
...
# Sem alarm_topic_ref em staging — sem SNS de ops aqui; só métrica/dashboard.
